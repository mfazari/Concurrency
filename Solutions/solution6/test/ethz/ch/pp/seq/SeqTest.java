...
public class SeqTest {
	  ...
	  @Test
	  public void testMultiSmallOpt() {
	    Assert.assertEquals(new Sequence(2, 3), LongestCommonSequenceMultiOptimal.longestCommonSequence(new int[]{1, 2, 3, 3, 2}));
	    Assert.assertEquals(new Sequence(0, 0), LongestCommonSequenceMultiOptimal.longestCommonSequence(new int[]{1, 2, 3, 2}));
	    Assert.assertEquals(new Sequence(0, 0), LongestCommonSequenceMultiOptimal.longestCommonSequence(new int[]{1}));
	    Assert.assertEquals(new Sequence(0, 2), LongestCommonSequenceMultiOptimal.longestCommonSequence(new int[]{1, 1, 1}));
	    Assert.assertEquals(new Sequence(0, 5), LongestCommonSequenceMultiOptimal.longestCommonSequence(new int[]{1, 1, 1, 1, 1, 1}));
	  }

	  @Test
	  public void testMultiRandOpt() {
		  DatasetGenerator gen = new DatasetGenerator();
		  for (int j = 0; j < 20; j++) {
			  int[] input = gen.generate();
			  Sequence result = LongestCommonSequenceMultiOptimal.longestCommonSequence(input);
	  ...
	  @Test
	  public void testCompareOpt() {
		  DatasetGenerator gen = new DatasetGenerator(20);
		  for (int j = 0; j < 1000; j++) {
			  int[] input = gen.generate();
			  Assert.assertEquals(
					  Arrays.toString(input),
					  LongestCommonSequenceMultiOptimal.longestCommonSequence(input),
					  LongestCommonSequenceMulti.longestCommonSequence(input)
					  );
		  }
	  }
